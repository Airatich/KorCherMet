...
def clicks_for_pipe_companies():
    driver.find_element(By.XPATH,
                        "/html/body/table/tbody/tr[5]/td/table[2]/tbody/tr[4]/td/table[1]/tbody/tr[1]/td[2]/select/option[5]").click()  # отрасль предприятия - трубные предприятия
    driver.find_element(By.XPATH,
                        '/html/body/table/tbody/tr[5]/td/table[2]/tbody/tr[4]/td/table[1]/tbody/tr[3]/td[2]/input[2]').click()  # показать
    selected = [3, 6, 7, 9, 11]
    for i in selected:
    ...
    driver.find_element(By.XPATH,
                        '/html/body/table/tbody/tr[5]/td/table[2]/tbody/tr[4]/td/table[2]/tbody/tr[64]/td[2]/input[1]').click()  # выбрать
...
def clicks_for_steel():
    ...
    driver.find_element(By.XPATH,
                        '/html/body/table/tbody/tr[5]/td/table[2]/tbody/tr[2]/td/table/tbody/tr/td/a').click()  ## Вернуться к форме запроса
...
def to_excel(month_inp, year):
    ...
    data = data[0]
    data.to_excel(rf'{settings.path_to_project}/Готовые excel/Сталь {month_inp.zfill(2)}.{year}.xlsx', index=False)
    table = pd.read_excel(f'{settings.path_to_project}/Готовые excel/Сталь {month_inp.zfill(2)}.{year}.xlsx', sheet_name='Sheet1', header=0, skiprows=17)
    table.dropna(axis=0, how="all", inplace=True)
    table.drop(table.columns[[0]], axis=1,inplace=True)
    table.to_excel(
        rf'{settings.path_to_project}/Готовые excel/Сталь {month_inp.zfill(2)}.{year}.xlsx',
        index=False)
...
def main_steel(year, month_inp):
    ...
    print(f"Сталь {month_inp.zfill(2)}.{year} - лежит в папке 'Готовые excel'")
